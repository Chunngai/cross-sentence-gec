...
@register_model('document_level_transformer')
# [CONTEXT]
# class TransformerModel(FairseqModel):
class DocumentLevelTransformerModel(FairseqDualEncoderModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--auxencoder-embed-path', type=str, metavar='STR',
                            help='path to pre-trained auxiliary encoder embedding')
        parser.add_argument('--auxencoder-embed-dim', type=int, metavar='N',
                            help='auxiliary encoder embedding dimension')
        parser.add_argument('--auxencoder-ffn-embed-dim', type=int, metavar='N',
                            help='auxiliary encoder embedding dimension for FFN')
        parser.add_argument('--auxencoder-layers', type=int, metavar='N',
                            help='num auxiliary encoder layers')
        parser.add_argument('--auxencoder-attention-heads', type=int, metavar='N',
                            help='num auxiliary encoder attention heads')
        parser.add_argument('--auxencoder-normalize-before', action='store_true',
                            help='apply layernorm before each auxiliary encoder block')
        parser.add_argument('--auxencoder-learned-pos', action='store_true',
                            help='use learned positional embeddings in the auxiliary encoder')
    ...
    def build_model(cls, args, task):
        ...
        # [CONTEXT]/
        if not hasattr(args, 'max_context_positions'):
            args.max_context_positions = 1024
        ...
        if args.share_all_embeddings:
            ...
            if not (ctx_dict == src_dict == tgt_dict):
                raise ValueError('--share-all-embeddings requires a joined dictionary')
            # [CONTEXT]/
            # if args.encoder_embed_dim != args.decoder_embed_dim:
            if not (args.auxencoder_embed_dim == args.encoder_embed_dim == args.decoder_embed_dim):
                raise ValueError(
                    '--share-all-embeddings requires --encoder-embed-dim to match --decoder-embed-dim')
            # [CONTEXT]/
            # if args.decoder_embed_path and (
            #         args.decoder_embed_path != args.encoder_embed_path):
            if args.decoder_embed_path and not (
                    args.auxencoder_embed_path == args.encoder_embed_path == args.decoder_embed_path):
                raise ValueError('--share-all-embeddings not compatible with --decoder-embed-path')
            encoder_embed_tokens = build_embedding(
                src_dict, args.encoder_embed_dim, args.encoder_embed_path
            )
            decoder_embed_tokens = encoder_embed_tokens
            auxencoder_embed_tokens = encoder_embed_tokens  # [CONTEXT]/
            ...
            # [CONTEXT]/
            auxencoder_embed_tokens = build_embedding(
        ...
        decoder = DocumentLevelTransformerDecoder(args, tgt_dict, decoder_embed_tokens)
        # [CONTEXT]
        # return TransformerModel(encoder, decoder)
        return DocumentLevelTransformerModel(auxencoder, encoder, decoder)
...
# [CONTEXT]
# class TransformerEncoder(FairseqEncoder):
class DocumentLevelTransformerEncoder(FairseqEncoder):
    """
    Transformer encoder consisting of *args.encoder_layers* layers. Each layer
    is a :class:`TransformerEncoderLayer`.
...
class DocumentLevelTransformerDecoder(FairseqIncrementalDecoder):
    """
...
@register_model_architecture('document_level_transformer', 'document_level_transformer')
def base_architecture(args):
    ...
    args.auxencoder_embed_dim = getattr(args, 'auxencoder_embed_dim', 512)
    args.auxencoder_ffn_embed_dim = getattr(args, 'auxencoder_ffn_embed_dim', 2048)
    args.auxencoder_layers = getattr(args, 'auxencoder_layers', 6)
    args.auxencoder_attention_heads = getattr(args, 'auxencoder_attention_heads', 8)
    args.auxencoder_normalize_before = getattr(args, 'auxencoder_normalize_before', False)
    args.auxencoder_learned_pos = getattr(args, 'auxencoder_learned_pos', False)
...
# @register_model_architecture('document_level_transformer', 'document_level_transformer_wmt_en_de_big_t2t')

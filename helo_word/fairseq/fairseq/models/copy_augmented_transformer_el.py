    ...
    **One model dimension is reserved Outputs edit labels for each input token.**
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=True):
        ...
        if self.predict_edit_labels:
            ...
            # (NOTE) copy_augmented_transformer: exe here.
...
class CopyAugmentedTransformerELDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        """
        # (NOTE) Computes copy_attn, copy_attn_weights, alpha.
        if self.copy_attention is not None:
            assert encoder_out is not None
            # attn_output: T_dec x B x C, copy_scores: B x T_dec x T_enc
            # (NOTE) The same as enc-dec attn except for the lack of `incremental_state`,
            # (NOTE) and for `need_weights` (always True here).
            # (NOTE)
            """
            The first 3 rows of an example of `copy_scores`:
            
            [[0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000,
             0.0196, 0.0225, 0.0204, 0.0179, 0.0197, 0.0210, 0.0216, 0.0177, 0.0204,
             0.0233, 0.0210, 0.0194, 0.0207, 0.0193, 0.0268, 0.0171, 0.0202, 0.0154,
             0.0215, 0.0197, 0.0264, 0.0231, 0.0216, 0.0222, 0.0185, 0.0203, 0.0205,
             0.0238, 0.0171, 0.0215, 0.0231, 0.0161, 0.0248, 0.0210, 0.0225, 0.0208,
             0.0234, 0.0194, 0.0163, 0.0195, 0.0193, 0.0226, 0.0234, 0.0200, 0.0208,
             0.0230, 0.0188, 0.0247],
            [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000,
             0.0218, 0.0239, 0.0186, 0.0166, 0.0207, 0.0196, 0.0223, 0.0176, 0.0251,
             0.0272, 0.0215, 0.0223, 0.0241, 0.0198, 0.0226, 0.0183, 0.0191, 0.0180,
             0.0219, 0.0201, 0.0190, 0.0230, 0.0188, 0.0195, 0.0183, 0.0203, 0.0197,
             0.0253, 0.0166, 0.0236, 0.0220, 0.0185, 0.0265, 0.0227, 0.0217, 0.0169,
             0.0190, 0.0230, 0.0169, 0.0214, 0.0172, 0.0214, 0.0244, 0.0178, 0.0206,
             0.0247, 0.0198, 0.0204],
             [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000,
             0.0221, 0.0228, 0.0212, 0.0166, 0.0183, 0.0191, 0.0206, 0.0191, 0.0223,
             0.0242, 0.0230, 0.0179, 0.0205, 0.0176, 0.0226, 0.0179, 0.0226, 0.0212,
             0.0216, 0.0219, 0.0216, 0.0201, 0.0189, 0.0227, 0.0218, 0.0227, 0.0202,
             0.0209, 0.0182, 0.0210, 0.0198, 0.0191, 0.0271, 0.0198, 0.0245, 0.0210,
             0.0206, 0.0231, 0.0179, 0.0209, 0.0189, 0.0212, 0.0233, 0.0203, 0.0195,
             0.0215, 0.0172, 0.0230],
            """
            ...
            # (NOTE) 512 d -> 1 d on the 3rd dimension (emb dim).
        ...
        """
        # (NOTE) It means for each word in each batch (sentence),
        # (NOTE) which word in the vocab should it choose (GENERATE).
        gen_probs = None
        # (NOTE) It means for each word in each batch (sentence),
        # (NOTE) which word in the vocab should it choose (COPY).
        copy_probs = None
        self.num_batches += 1
        ...
        if self.copy_attention is not None:
            ...
            # (NOTE) copy_prob is applied softmax() after calculating qk and masking,
            # (NOTE) thus softmax is not needed here.
...
def compute_copy_probs(copy_scores, src_tokens, vocab_size):
    ...
    # (NOTE) Creates a container for storing the copy probs.
    # (NOTE)
    """
    OUT:
    `copy_probs`: B x T x V  # All zeros.
    """
    copy_probs = copy_scores.new_zeros((batch_size, tgt_len, vocab_size),
                                       requires_grad=copy_scores.requires_grad)

    # (NOTE) Note that elems in `src_tokens` are indices of words in V.
    #
    # (NOTE) It means for each output word in each batch (output sentence),
    # (NOTE) which words the output word can choose (copy).
    # (NOTE) Only the words that appear in the input sentence can be chosen.
    #
    # (NOTE) Basically it is for the requirement of `scatter_add()`.
    # (NOTE)
    """
    IN: `src_tokens`: B x S
    
    OUT:
    `src_tokens_expanded`: B x T x S
    """
    src_tokens_expanded = src_tokens.unsqueeze(1).expand(-1, tgt_len, -1)

    # (NOTE)
    """
    The first 10 rows of `copy_probs.scatter_add(2, src_tokens_expanded, copy_scores)[1][0]`:
    
    [0.0000, 0.0000, 0.0247, 0.0000, 0.0000, 0.0000, 0.0619, 0.0202, 0.0000,
    0.0657, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0188, 0.0000, 0.0000,
    0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0391,
    0.0000, 0.0000, 0.0171, 0.0629, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000,
    0.0000, 0.0000, 0.0225, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000,
    0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000,
    0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000,
    0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000,
    0.0000, 0.0413, 0.0000, 0.0000, 0.0193, 0.0000, 0.0000, 0.0000, 0.0000,
    0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000,
    """

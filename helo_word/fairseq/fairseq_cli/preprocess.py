...
def main(args):
    ...
    if args.joined_dictionary:
        ...
        tgt_dict = src_dict
        ctx_dict = src_dict  # [CONTEXT]
    else:
        if args.srcdict:
            src_dict = task.load_dictionary(args.srcdict)
            ctx_dict = task.load_dictionary(args.srcdict)  # [CONTEXT]
        else:
            assert args.trainpref, "--trainpref must be set if --srcdict is not specified"
            src_dict = build_dictionary([train_path(args.source_lang)], src=True)
            ctx_dict = build_dictionary([train_path('ctx')], src=True)  # [CONTEXT]
    ...
    ctx_dict.save(dict_path('ctx'))  # [CONTEXT]

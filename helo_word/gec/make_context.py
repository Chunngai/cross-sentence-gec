...
def get_documents(document_file_paths: List[str], m2_file_paths: List[str], source_file_path: str) -> List[List[str]]:
    """Get documents according to the dataset.

    Args:
        document_file_paths: Paths of files containing documents. (.json / .sgml / "entries.train")
        m2_file_paths: Paths of .m2 files containing tokenized sentences.
        source_file_path: The path of {DATASET_NAME}.ori.
...
def make_context(source_file_path: str, document_file_paths: List[str], m2_file_paths: List[str],
                 previous_sentences_number: int = 1, following_sentences_number: int = 0):
    """Create a context file for a dataset.

    Args:
        source_file_path: The path of the source file {DATASET_NAME}.ori.
        document_file_paths: Paths of files containing documents. (.json / .sgml / "entries.train")
        m2_file_paths: Paths of .m2 files containing tokenized sentences.
    ...
    source_file_path_without_extension = splitext(source_file_path)[0]
    context_file_path = f"{source_file_path_without_extension}.ctx"
    ...
    with open(context_file_path, "w") as context_file:
        for source_sentence in source_sentences:
            ...
            if source_sentence != previous_source_sentence:
                ...
                while source_sentence not in current_document_masked:
                ...
                try:
                    document_level_index = current_document_masked.index(source_sentence)
                except ValueError:
...
if __name__ == '__main__':
    ...
    for dataset in [
        ...
        make_context(eval(f"{dataset}_ori_path"),

...
testset=$1  # conll13st-test or conll14st-test
model_path=$2
DATA_DIR=$3
...
REF=./data/test/$testset/$testset.m2  # Trg.

OUTPUT=$OUT_DIR/$testset.out
LOGFILE=$OUT_DIR/decode.$testset.log

TMP_DIR=$OUT_DIR/tmp.$testset$suffix.`date +%s`  # $suffix: undefined?
mkdir $TMP_DIR
...
CUDA_VISIBLE_DEVICES=$DEVICE python3 $FAIRSEQPY/interactive_multi.py --no-progress-bar --path $models --beam $beam --nbest $beam --replace-unk --source-lang src --target-lang trg --input-files $TMP_DIR/input.src $TMP_DIR/input.ctx --num-shards $threads --task translation_ctx  $DATA_DIR  > $OUTPUT

# Extracts outputs.
#
# `grep ^H`: Extracts all lines starting with an 'H'.
#
# `python3 -c "import sys; x = sys.stdin.readlines(); x = ' '.join([ x[i] for i in range(len(x)) if i%$nbest == 0 ]); print(x)"`: 
#
# `cut -f3`: Extracts the 1st col.
#
# `sed 's|@@ ||g'`: replace "@@ " by "".
#
# `sed '$ d'`: Rm the final blank line.
